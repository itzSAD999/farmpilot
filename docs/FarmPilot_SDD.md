...
- Shows the Twi text instead of English only when the signed-in farmer's `profiles.preferred_language = 'tw'` (Profile.tsx). The control is labelled **"Advice Language,"** not the more generic "Language" the original spec assumed — a plain "Language" setting would have implied the whole app switches (menus, buttons, page titles), when in fact only the Estimate Report's advice text and audio are affected; this was flagged directly during review and fixed by both renaming the field and adding a one-line description under it stating the scope explicitly. The Twi option itself is labelled "Twi (Akan)," since farmers may know the language by a different name than the bare linguistic term. Changing this preference takes effect immediately across the app with no reload, because both Profile.tsx and EstimateReport.tsx read the same React Query cache key (`['profile', userId]`) and Profile's save mutation invalidates it on success.
...
**The one exception to the `getTwiAdvice()` path: the public Landing page.** `advice_rules`/`advice_translations` are readable only `to authenticated` (§19.4) — correct for the app itself, but it means `getTwiAdvice()` cannot be called from `/welcome`, which by definition has no signed-in visitor. The Landing page's interactive Twi demo instead plays the fertiliser clip's Storage URL directly — the bucket itself is public-read, so the audio file loads with no session regardless of whether the row lookup that would normally resolve that URL could run. This is a narrow, deliberate exception, not a workaround worth generalising: it exists only because this one page's audience is, by design, always logged out.
