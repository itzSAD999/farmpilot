...
**Row-level security as an authorisation model.** Most web applications enforce "a user may only see their own data" inside the application's own code — a check added by hand to every place the code fetches data, easy to forget in a new one. PostgreSQL's **row-level security (RLS)** moves that rule into the database itself instead: a permission rule is attached directly to a table, and the database checks it on every single read and write, no matter which part of the code asked for the data. In other words, the safety check cannot be bypassed by a bug in the app, because it isn't the app enforcing it. This is the approach FarmPilot uses throughout (§3.2, §4.2), and it is the standard recommendation for applications, like this one, that let the frontend talk almost directly to the database (through Supabase's auto-generated data layer, PostgREST) instead of going through a hand-written server in the middle.

**Database-resident business logic.** Instead of writing the cost-estimation calculation as ordinary application code running on a separate server, FarmPilot writes it as a function that lives *inside* the database itself (in PostgreSQL's own scripting language, PL/pgSQL — see §4.1). This is a deliberate trade-off. The upside: the calculation runs right next to the data it needs (no separate step of pulling every cost record out over the network first), the same row-level-security rule that protects the raw data automatically protects the calculation too, and there is one fewer separate piece of infrastructure to build, deploy, and keep secure. The downside: PL/pgSQL is a less common, harder-to-test language than the general-purpose ones used elsewhere in the project. The team accepted that trade-off deliberately, and it is discussed openly rather than hidden (§4.1, §5.2).
...
`auth.users` (Supabase's built-in table of registered accounts, one row per farmer) sits above `farms` and is left off the diagram to save space. Every farm row is linked to exactly one account, and that link is set up so that deleting the account automatically deletes everything that belongs to it (the farm, its seasons, its costs) — the database does this cleanup itself, so the application never has to remember to do it in several different places. The `is_actual` flag on `estimate_lines` marks whether a category is one the farmer has genuinely recorded spending on this season, versus one still showing a predicted figure (§4.2.3) — this flag was added partway through building the system, once testing showed the original design could not tell the two apart correctly (§4.2.4).
...
The table below lists what each part of the system is built with. In short: the screen a farmer sees is built in React (a common way to build interactive web pages); it talks to a managed PostgreSQL database (via Supabase, a hosting service that also generates a ready-made data API and handles login/sign-up); and the estimation calculation itself is written as a function stored inside that database, in PostgreSQL's own scripting language, PL/pgSQL, rather than as a separate program.
...
Clicking *Generate Estimate* runs a single function stored inside the database, `generate_estimate(season_id)`. For each of the eight cost categories (seeds, fertiliser, and so on), it does three things, in order:

1. **Decide what to predict from, for categories with nothing recorded yet.** If the farm has already completed an earlier season of the same crop, the farmer's own past average cost per acre is used as the prediction (this is called the `history` method). If not — the farmer's very first season of that crop — the function falls back to the standard MoFA-derived benchmark rate instead (the `benchmark` method). Either way, a prediction is always available, even on day one.
2. **Check whether the farmer has actually spent money on that category this season.** If they have recorded a real cost, that real figure is used in the report instead of the prediction, and the category is marked as "actual" (`is_actual = true`) rather than "predicted." A category the farmer hasn't touched yet stays as a prediction and is never treated as if it were real spending.
3. **Only for categories marked "actual," check for overspending.** The real recorded figure is compared against the fixed benchmark rate for that category. If it is more than 30% above the benchmark (the threshold can be configured), the category is flagged as overspending, the amount of money that could be saved is calculated, and a specific, practical suggestion for that category is attached (drawn from a table of stored advice, `advice_rules`).
...
The very first version of `generate_estimate` had a real bug, and it is worth explaining plainly because it is the most important fix made in the whole project. On a farmer's first season of a crop (no history yet), the "what did you actually spend" figure and the "what's the standard benchmark" figure it was supposed to be checked against were, by mistake, being calculated from the exact same source. That meant the two numbers were always identical — a figure compared against an exact copy of itself always shows 0% difference — so the system could never flag an overspend, no matter how much the farmer had actually overpaid. The bug would not have shown up just by looking at the screen; it only became visible when the engine was tested directly against a known, deliberately-inflated cost figure and the flag failed to appear (§4.3). The fix was step 2 in §4.2.3 above: explicitly checking whether the farmer has really recorded a cost for that category this season, and only comparing *that real figure* — never a prediction — against the benchmark. This one correction is what makes the overspend-detection feature actually work as intended, rather than silently doing nothing. The full technical write-up, including the exact test numbers before and after the fix, is in the companion System Design Document, §9.2.
...
**Budgets** answer a different question: not "is this above the national-average benchmark," but "is this above what *I* am willing to spend." A farmer can cap spending at five independent levels — one category within one active season (the original Category Budgets), a crop's total across every season it's grown, the whole farm, a category across the whole farm regardless of crop, or a category within one specific crop (e.g. "for Maize: no more than GHS 300 on labour"). Every level shows a progress bar and how much has actually been spent against it, and the cost-entry form warns, before a cost is saved, if it would push *any* applicable cap over its limit — not just the season-level one. This is deliberately independent of the benchmark comparison throughout: a category can be within the MoFA-derived benchmark and still over a farmer's own budget, or the reverse. A dedicated Budgets page (`/budgets`) lists every cap with search and an over-budget/not-set filter, and the AI assistant (FarmBot) can also see and discuss them.
...
| T11 | Offline cost entry | Record a cost item with the network disabled, then reconnect | Entry queues locally, flushes automatically on reconnect, no duplicate created on a retried flush | Pass (post-fix) — the offline queue itself was already fully built, but nothing in the recording form actually called it, so this originally could not have passed as described; found and wired up during the automated-testing pass, see Development Log Issue #40 |
...
Tests T2, T6, T7, T10, and T11 each led directly to a correction in the implementation rather than only confirming existing behaviour — a reflection of testing being carried out throughout implementation rather than only after it (§3.1).

**Automated tests.** T1–T12 above were run by hand — a real limitation, since manual evidence cannot be re-checked automatically after a later change. To address this directly, an automated suite was added, split into two tiers to keep the fast one always runnable: `npm test` (76 tests) covers pure client-side logic with no network dependency — pesewa/cedi conversion and round-tripping, category-list and Ghana-region/district consistency, error-message mapping for every API module, and the Weekly Check-in's proportional-by-acreage split rule (extracted into a standalone function specifically so it could be unit tested, with the exact 1-acre/5-acre worked example from earlier testing now a permanent regression test); `npm run test:integration` (64 tests) hits the real, linked database against throwaway test accounts and farms created and torn down within each run, covering every `src/api/*.ts` module — recording, estimating, comparing, budgeting, dashboards, the guide library, authentication, and notifications — including a database trigger that had never been exercised by anything before: an "Overspend Alert" notification written automatically the instant a category is flagged, with no application code involved. The first run of the integration suite found a genuine, previously-unknown defect: two benchmark functions serving the same crop/acreage disagreed by a few pesewas due to differing rounding order, traced to one of them being dead code left over from an earlier version of Cost Lab and removed once confirmed unused. A later run surfaced a second, purely operational finding: running the suite repeatedly in quick succession trips Supabase Auth's own sign-up rate limit, confirmed with a standalone probe script to be the hosting platform's own protection rather than an application defect, and addressed by running test files sequentially and sharing throwaway accounts across assertions that don't need a fresh one. The most significant finding, however, came from writing tests for the one area the suite had not yet reached — offline support (`src/lib/offline/`): the write-queue that is supposed to let a farmer record a cost with no signal was fully built, but nothing in the actual recording form ever called the function that puts a write into it, so the feature this report and the README describe did not function for its real use case. This is corrected in this submission (§4.2.7, T11, and Development Log Issue #40) rather than left as a known gap, precisely because writing the automated suite is what found it. That real bugs kept surfacing on the very first runs of new tests — not edge cases, but in one case a core feature that had never actually been exercised as built — is itself the argument for having written them — see the Development Log, Issues #34–40, for the full account.
...
- `supabase/migrations/022–024` — the four additional budgeting tiers (crop total, farm total, farm-wide category, crop×category) and their status views (§4.2.7).
...
an index into all twelve architecture decisions, a forty-eight-item issue
